...
- 修复 B 站新视频发布初期 AI 字幕接口缓存污染导致视频总结内容不符（与标题脱节）的问题。
- 增强 `get_bilibili_ai_subtitle` 的有效性校验（增加时长覆盖率下限 $\ge 65\%$、起始时间及语音密度校验），校验不通过时自动无缝回退至音频下载与必剪 BCut ASR 精准转写。
- 导出 `build_combined_summary_prompt` 辅助函数并完善相关单元测试。

## v2.4.7

- 修复视频简介过长被 100 字符强制截断导致网盘链接等信息丢失的问题。
- 视频简介默认不截断（完整展示），并在设置与 WebUI 面板中新增 `detect_desc_max_len`（简介截断最大字数）配置项，支持自定义截断上限（0 为不限制）。
