...
def parse_event(event: object) -> MessageContext:
    ...
    if message_obj is not None:
        # ✅ 修复：优先使用 message（组件列表），保留结构化数据
        raw_payload = getattr(message_obj, "message", None)
    ...
    for idx, comp in enumerate(components):
        ...
        # ✅ 改进 JSON 卡片处理：保存原始的 data 字段内容
        if comp_type == "json" or "json" in comp_name:
            comp_data = getattr(comp, "data", None)
            if isinstance(comp_data, dict):
                inner_data = comp_data.get("data")
                if isinstance(inner_data, str):
                    json_card_text = inner_data
                else:
                    json_card_text = str(comp_data)
            elif isinstance(comp_data, str):
                json_card_text = comp_data
            else:
                comp_str = str(comp)
                if comp_str:
