...
class SearchService:
    ...
    async def process_bv_list(
        ...
        *,
        ...
        # 🚀 新增：AI智能搜索转写文本历史文件夹自动强删熔断机制（超过3天删除）
        try:
            now = time.time()
            if self._search_dir.exists():
                for sub_dir in self._search_dir.iterdir():
                    if sub_dir.is_dir() and (now - sub_dir.stat().st_mtime > 259200):  # 3天 = 259200秒
                        await asyncio.to_thread(shutil.rmtree, sub_dir, ignore_errors=True)  # 改为异步线程执行
                        logger.info(f"已自动清理过期AI搜索文件夹: {sub_dir.name}")
        except Exception as e:
            logger.warning(f"全自动熔断清除过期AI搜索记录文件夹异常: {e}")
