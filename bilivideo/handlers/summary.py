...
async def handle_summary(services: BiliVideoServices, event: object) -> AsyncIterator[object]:
    ...
    if remaining > 0:
        yield event.plain_result(f"⏳ 操作太频繁，请等 {remaining} 秒后再试")
        return
    ...
    if detect_platform(video_url) == "coolapk":
        yield event.plain_result("⏳ 正在获取酷安动态并生成总结，请稍候...")
        services.cooldown.punch(cooldown_key)
        try:
            post = await fetch_coolapk_post(video_url)
...
async def handle_latest_video(services: BiliVideoServices, event: object) -> AsyncIterator[object]:
    ...
    if not mid:
        ...
        if uploader is None:
            ...
            return
        mid = uploader.mid
        yield event.plain_result(f"✅ 找到UP主【{uploader.name}】(UID:{mid})")
