...
class _Services:
    def __init__(self, config: PluginConfig, providers: list[_Provider]) -> None:
        ...
        self.llm = _LLM()
        self.raw_config = {"llm": {"llm_provider": "astrbot"}}
...
async def test_switch_to_valid_id() -> None:
    ...
    assert services.config.llm_provider_id == "ds"
    assert services.raw_config["llm"]["llm_provider_id"] == "ds"
    assert "已切换" in out[0]
...
async def test_reset_restores_default() -> None:
    ...
    assert services.config.llm_provider_id == ""
    assert services.raw_config["llm"]["llm_provider_id"] == ""
    assert "默认" in out[0]
