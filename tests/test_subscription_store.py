...
@pytest.mark.asyncio
async def test_add_subscription_merges_existing_types(tmp_path) -> None:
    mgr = SubscriptionManager(str(tmp_path))
    await mgr.add_subscription("origin1", "12345", "Foo", sub_types=["动态"])

    assert await mgr.add_subscription("origin1", "12345", "Foo", sub_types=["视频"]) == 2

    subs = await mgr.get_subscriptions("origin1")
    assert len(subs) == 1
    assert subs[0].sub_types == ["动态", "视频"]


@pytest.mark.asyncio
async def test_auto_summary_permission_owner_is_persisted(tmp_path) -> None:
    mgr = SubscriptionManager(str(tmp_path))
    await mgr.add_subscription(
        ...
        "Foo",
        auto_summary=True,
        auto_summary_user_id="67890",
    )
    sub = (await mgr.get_subscriptions("origin1"))[0]
    assert sub.auto_summary
    assert sub.auto_summary_user_id == "67890"
